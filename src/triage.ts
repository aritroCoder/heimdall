...
const AUTHOR_LABELS: readonly string[] = [
    AUTHOR_LABEL_MEMBER,
    AUTHOR_LABEL_CONTRIBUTOR,
    AUTHOR_LABEL_NEW,
];
...
export const SIZE_LABEL_COLORS: readonly string[] = [
    '3cbf00',
    '5d9801',
    'fbca04',
    'ff9500',
...
const GENERIC_TITLE_RE =
    /^(update[ds]?|fix(e[ds])?|changes?|misc|improvements?|refactor(ed)?|cleanup|wip|add(ed)?|remove[ds]?|delete[ds]?|modify|modifie[ds]|tweak(ed)?|adjust(ed)?|bump(ed)?)\b/i;
const GENERIC_COMMIT_RE =
    /^(fix|update|changes?|misc|refactor|cleanup|wip|address review comments|apply suggestions?)($|[:\s])/i;
const AI_DISCLOSURE_RE =
    /(generated|written)\s+(?:primarily\s+)?(?:by|with)\s+(?:ai|chatgpt|claude|copilot)/i;
const TEST_FILE_RE =
...
function normalizeConfig(
    inputConfig: TriageConfigInput | null | undefined
): TriageConfig {
    const config = { ...DEFAULT_CONFIG, ...inputConfig };

    return {
        ...config,
        aiSlopThreshold: parseInteger(
            config.aiSlopThreshold,
            DEFAULT_CONFIG.aiSlopThreshold
        ),
        lowEffortThreshold: parseInteger(
            config.lowEffortThreshold,
            DEFAULT_CONFIG.lowEffortThreshold
        ),
        minFindingsForLabel: parseInteger(
            config.minFindingsForLabel,
            DEFAULT_CONFIG.minFindingsForLabel
        ),
        trustedAuthors: Array.isArray(config.trustedAuthors)
            ? config.trustedAuthors
            : [],
        trustedTitlePatterns: Array.isArray(config.trustedTitlePatterns)
            ? config.trustedTitlePatterns
            : [],
        sizeLabels: Array.isArray(config.sizeLabels)
            ? config.sizeLabels
            : DEFAULT_CONFIG.sizeLabels,
...
export function buildConfigFromEnv(env: EnvMap): TriageConfig {
    ...
    if (env.TRIAGE_AI_SLOP_THRESHOLD)
        overrides['aiSlopThreshold'] = env.TRIAGE_AI_SLOP_THRESHOLD;
    if (env.TRIAGE_LOW_EFFORT_THRESHOLD)
        overrides['lowEffortThreshold'] = env.TRIAGE_LOW_EFFORT_THRESHOLD;
    if (env.TRIAGE_AI_SLOP_LABEL)
        overrides['aiSlopLabel'] = env.TRIAGE_AI_SLOP_LABEL;
    if (env.TRIAGE_LOW_EFFORT_LABEL)
        overrides['lowEffortLabel'] = env.TRIAGE_LOW_EFFORT_LABEL;
    if (env.TRIAGE_HUMAN_REVIEWED_LABEL)
        overrides['humanReviewedLabel'] = env.TRIAGE_HUMAN_REVIEWED_LABEL;
    if (env.TRIAGE_TRUSTED_AUTHORS)
        overrides['trustedAuthors'] = parseCsv(env.TRIAGE_TRUSTED_AUTHORS);
    if (env.TRIAGE_TRUSTED_TITLE_REGEX)
        overrides['trustedTitlePatterns'] = parseCsv(
            env.TRIAGE_TRUSTED_TITLE_REGEX
        );
    if (env.TRIAGE_MIN_FINDINGS)
        overrides['minFindingsForLabel'] = env.TRIAGE_MIN_FINDINGS;
    if (env.TRIAGE_SIZE_THRESHOLDS)
        overrides['sizeThresholds'] = parseCsv(env.TRIAGE_SIZE_THRESHOLDS).map(
            Number
        );
    if (env.TRIAGE_SIZE_LABELS)
        overrides['sizeLabels'] = parseCsv(env.TRIAGE_SIZE_LABELS);
...
export function analyzePullRequest({
    ...
    const hasSource = fileNames.some(
        (filename) => isSourceFile(filename) && !isDocsOrConfigFile(filename)
    );
    const docsOrConfigOnly =
        fileNames.length > 0 &&
        fileNames.every((filename) => isDocsOrConfigFile(filename));
    ...
    const genericCommitCount = commitHeadlines.filter((headline) =>
        GENERIC_COMMIT_RE.test(headline)
    ).length;
    const genericCommitRatio =
        commitHeadlines.length > 0
            ? genericCommitCount / commitHeadlines.length
            : 0;
    const normalizedCommits = commitHeadlines.map((headline) =>
        normalizeCommitMessage(headline)
    );
    const uniqueCommitCount = new Set(normalizedCommits).size;
    const uniqueCommitRatio =
        commitHeadlines.length > 0
            ? uniqueCommitCount / commitHeadlines.length
            : 1;
    ...
    if (bodyLength < 40) {
        lowEffortFindings.push(
            buildFinding(
                ...
                'low-effort',
                28,
                'PR description is under 40 characters.'
            )
        );
        ...
        lowEffortFindings.push(
            buildFinding(
                ...
                'low-effort',
                15,
                'PR description is under 120 characters.'
            )
        );
    }
    ...
    if (title.length < 12 || GENERIC_TITLE_RE.test(title)) {
        lowEffortFindings.push(
            buildFinding(
                ...
                'low-effort',
                10,
                'PR title is very generic or too short.'
            )
        );
        aiSlopFindings.push(
            buildFinding(
                'generic-title-ai-signal',
                'ai-slop',
                8,
                'Generic PR title is a weak AI-slop signal.'
            )
        );
    }
    ...
    if (hasSource && !hasTests && totalLinesChanged >= 300) {
        lowEffortFindings.push(
            buildFinding(
                'no-tests-large-change',
                'low-effort',
                24,
                'Large source-code change without tests.'
            )
        );
        aiSlopFindings.push(
            buildFinding(
                'no-tests-large-change-ai-signal',
                'ai-slop',
                16,
                'Large source-code change without tests.'
            )
        ...
        lowEffortFindings.push(
            buildFinding(
                'no-tests-medium-change',
                'low-effort',
                12,
                'Multi-file source change without tests.'
            )
        );
    }
    ...
    if (totalLinesChanged >= 1200) {
        lowEffortFindings.push(
            buildFinding(
                ...
                'low-effort',
                12,
                'PR changes at least 1,200 lines.'
            )
        );
    ...
    if (totalLinesChanged <= 10 && fileCount <= 2 && bodyLength < 120) {
        lowEffortFindings.push(
            buildFinding(
                ...
                'low-effort',
                15,
                'Trivially small change with minimal context.'
            )
        );
    }
    ...
    if (commitHeadlines.length >= 2 && genericCommitRatio === 1) {
        ...
        aiSlopFindings.push(
            buildFinding(
                'mostly-generic-commits',
                'ai-slop',
                16,
                'Most commit messages are generic.'
            )
        );
    }
    ...
    if (commitHeadlines.length >= 4 && uniqueCommitRatio < 0.6) {
        aiSlopFindings.push(
            buildFinding(
                'repetitive-commit-patterns',
                'ai-slop',
                10,
                'Commit messages are repetitive.'
            )
        );
    }
    ...
    if (fileCount >= 8 && churnPerFile >= 200) {
        aiSlopFindings.push(
            buildFinding(
                ...
                'ai-slop',
                18,
                'High average churn per changed file.'
            )
        );
    } else if (fileCount >= 5 && churnPerFile >= 120) {
        aiSlopFindings.push(
            buildFinding(
                'moderate-churn-per-file',
                'ai-slop',
                10,
                'Moderately high churn per changed file.'
            )
        );
    }
...
function renderCategory(
    name: string,
    categoryResult: TriageCategoryResult
): string {
    const lines = [
        `### ${name}`,
        '',
        `- Score: **${categoryResult.score}/100** (threshold: ${categoryResult.threshold})`,
    ];
...
function isTrustedAuthor(
    author: string,
    config: Pick<TriageConfig, 'trustedAuthors'>
): boolean {
    return config.trustedAuthors
...
function classifyAuthorCategory(
...
async function ensureManagedLabelsExist({
    ...
    const labelDefinitions = [
        {
            ...
            color: 'd93f0b',
            description:
                'Automated triage: low-effort pull request signals detected.',
        },
        {
            name: config.aiSlopLabel,
            color: 'b60205',
            description:
                'Automated triage: AI-slop pull request signals detected.',
        },
    ];
    ...
    for (let i = 0; i < config.sizeLabels.length; i++) {
        labelDefinitions.push({
            ...
            color:
                SIZE_LABEL_COLORS[i] ||
                SIZE_LABEL_COLORS[SIZE_LABEL_COLORS.length - 1],
    ...
    const labelDefinitions = [
        {
            ...
            color: '0e8a16',
            description:
                'PR author classification: repository member or collaborator.',
        },
        {
        ...
        },
        {
            ...
            color: '0366d6',
            description:
                'PR author classification: other or first-time author.',
        },
    ];
...
async function syncManagedLabels({
    ...
    const managedLabelSet = new Set([
        config.lowEffortLabel,
        config.aiSlopLabel,
        ...config.sizeLabels,
    ]);
    const existingSet = new Set(existingLabels);
...
function buildSkippedDuplicateDetection(
    skipReason: string
): DuplicateDetectionResult {
    return {
        checked: false,
...
async function runDuplicateCheckForPullRequest({
    ...
    if (
        effectiveDuplicateConfig.onlyOnOpened &&
        eventAction &&
        eventAction !== 'opened' &&
    ...
    if (
        ...
        log.warning(
            'Duplicate detection skipped: github.rest.pulls.list is unavailable in this client.'
        );
    ...
    if (duplicateResult.flagged) {
        ...
        if (commentBody) {
            log.info(
                `Posting duplicate comment on ${owner}/${repo}#${pullNumber} (matches=${duplicateResult.matches.length}).`
            );
            ...
            log.warning(
                `Duplicate flagged for ${owner}/${repo}#${pullNumber} but comment body was empty; skipping upsert.`
            );
        }
    } else if (duplicateResult.checked) {
        await deleteDuplicateComment({
            github,
            owner,
            repo,
            issueNumber: pullNumber,
...
export async function runTriageForPullRequest({
    ...
    try {
        if (currentLabelNames.includes(effectiveConfig.humanReviewedLabel)) {
            ...
            log.info(
                'Skipping triage because PR has the human-reviewed override label.'
            );
            return {
                skipped: true,
                ...
                analysis: null,
                desiredLabels: [],
                duplicateDetection: buildSkippedDuplicateDetection(
                    'triage-bypassed-human-reviewed'
                ),
            };
        }
        ...
        if (isTrustedAuthor(author, effectiveConfig)) {
            ...
            return {
                ...
                desiredLabels: [],
                duplicateDetection: buildSkippedDuplicateDetection(
                    'triage-bypassed-trusted-author'
                ),
            };
        }

        if (
            matchesTrustedTitlePattern(
                fullPullRequest.title || '',
                effectiveConfig
            )
        ) {
            await syncManagedLabels({
            ...
            log.info(
                'Skipping triage because title matches an allowlisted pattern.'
            );
            return {
                skipped: true,
        ...
        let duplicateDetection: DuplicateDetectionResult =
            buildSkippedDuplicateDetection('not-run');
        try {
            ...
            log.warning(
                `Duplicate detection failed for ${owner}/${repo}#${pullNumber}. Continuing triage.`
            );
            log.error(error);
        }
        ...
        const desiredLabels = analysis.bypassed
            ? []
            : getDesiredLabels(analysis, effectiveConfig);
        const hasTriageLabels =
            analysis.lowEffort.flagged || analysis.aiSlop.flagged;

        if (desiredLabels.length > 0) {
            await ensureManagedLabelsExist({
                github,
                owner,
                repo,
                config: effectiveConfig,
        ...
        if (processingCommentPosted) {
            try {
                ...
                log.warning(
                    `Failed to delete processing comment for ${owner}/${repo}#${pullNumber}.`
                );
                log.error(error);
            }
        }
