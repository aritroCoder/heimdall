...
export const DEFAULT_DUPLICATE_CONFIG: Readonly<DuplicateConfig> =
    Object.freeze({
        enabled: true,
        onlyOnOpened: false,
...
export function normalizeDuplicateConfig(
    inputConfig: DuplicateConfigInput | null | undefined
): DuplicateConfig {
    const config = { ...DEFAULT_DUPLICATE_CONFIG, ...inputConfig };

    return {
        enabled: parseBoolean(config.enabled, DEFAULT_DUPLICATE_CONFIG.enabled),
        onlyOnOpened: parseBoolean(
            config.onlyOnOpened,
            DEFAULT_DUPLICATE_CONFIG.onlyOnOpened
        ),
        maxOpenCandidates: clamp(
            parseInteger(config.maxOpenCandidates, 80),
            1,
            400
        ),
        maxMergedCandidates: clamp(
            parseInteger(config.maxMergedCandidates, 140),
            1,
            600
        ),
        maxCandidateComparisons: clamp(
            parseInteger(config.maxCandidateComparisons, 60),
            1,
            200
        ),
        mergedLookbackDays: clamp(
            parseInteger(config.mergedLookbackDays, 180),
            1,
            1000
        ),
        fileCountDeltaThreshold: clamp(
            parseInteger(config.fileCountDeltaThreshold, 8),
            0,
            100
        ),
        ...
        fileOverlapThreshold: clamp(
            parseNumber(config.fileOverlapThreshold, 0.7),
            0,
            1
        ),
        structuralSimilarityThreshold: clamp(
            parseNumber(
                config.structuralSimilarityThreshold,
                DEFAULT_DUPLICATE_CONFIG.structuralSimilarityThreshold
            ),
            0,
            1
        ),
        metadataSimilarityThreshold: clamp(
            parseNumber(
                config.metadataSimilarityThreshold,
                DEFAULT_DUPLICATE_CONFIG.metadataSimilarityThreshold
            ),
            0,
            1
        ),
        candidateFetchConcurrency: clamp(
            parseInteger(config.candidateFetchConcurrency, 4),
            1,
            12
        ),
        ...
        metadataVectorSize: clamp(
            parseInteger(config.metadataVectorSize, 256),
            32,
            2048
        ),
        maxReportedMatches: clamp(
            parseInteger(config.maxReportedMatches, 3),
            1,
            10
        ),
    };
...
export function buildDuplicateConfigFromEnv(env: EnvMap): DuplicateConfig {
    ...
    if (env.DUPLICATE_MAX_CANDIDATE_COMPARISONS) {
        overrides['maxCandidateComparisons'] =
            env.DUPLICATE_MAX_CANDIDATE_COMPARISONS;
    }
    ...
    if (env.DUPLICATE_FILE_COUNT_DELTA_THRESHOLD) {
        overrides['fileCountDeltaThreshold'] =
            env.DUPLICATE_FILE_COUNT_DELTA_THRESHOLD;
    }
    if (env.DUPLICATE_TOP_LEVEL_DIR_OVERLAP_THRESHOLD) {
        overrides['topLevelDirOverlapThreshold'] =
            env.DUPLICATE_TOP_LEVEL_DIR_OVERLAP_THRESHOLD;
    }
    if (env.DUPLICATE_FILE_OVERLAP_THRESHOLD) {
        overrides['fileOverlapThreshold'] =
            env.DUPLICATE_FILE_OVERLAP_THRESHOLD;
    }
    if (env.DUPLICATE_STRUCTURAL_SIMILARITY_THRESHOLD) {
        overrides['structuralSimilarityThreshold'] =
            env.DUPLICATE_STRUCTURAL_SIMILARITY_THRESHOLD;
    }
    if (env.DUPLICATE_METADATA_SIMILARITY_THRESHOLD) {
        overrides['metadataSimilarityThreshold'] =
            env.DUPLICATE_METADATA_SIMILARITY_THRESHOLD;
    }
    if (env.DUPLICATE_CANDIDATE_FETCH_CONCURRENCY) {
        overrides['candidateFetchConcurrency'] =
            env.DUPLICATE_CANDIDATE_FETCH_CONCURRENCY;
    }
    if (env.DUPLICATE_MAX_PATCH_CHARS_PER_FILE) {
        overrides['maxPatchCharactersPerFile'] =
            env.DUPLICATE_MAX_PATCH_CHARS_PER_FILE;
    }
...
export function buildPullRequestRepresentation({
    ...
    const normalizedDiffHash = toSha256(
        [
            'A',
            ...sortedAddedLines.map((line) => `+${line}`),
            'R',
            ...sortedRemovedLines.map((line) => `-${line}`),
        ].join('\n')
    );
    const patchFingerprint = toSha256(
        ['A', ...sortedAddedLines, 'R', ...sortedRemovedLines].join('\n')
    );
    ...
    return {
        ...
        metadataTokenVector: buildFeatureHashVector(
            metadataText,
...
export function evaluateCandidateSimilarity(
    ...
    const fileOverlap = jaccardSimilarity(
        currentRepresentation.fileSet,
        candidateRepresentation.fileSet
    );
    ...
    const fileCountDelta = Math.abs(
        currentRepresentation.fileCount - candidateRepresentation.fileCount
    );
    ...
    const patchIdMatch =
        ...
        currentRepresentation.patchFingerprint ===
            candidateRepresentation.patchFingerprint;
    const inversePatchMatch =
        currentRepresentation.patchFingerprint.length > 0 &&
        currentRepresentation.patchFingerprint ===
            candidateRepresentation.inversePatchFingerprint;
    const normalizedDiffHashMatch =
        currentRepresentation.normalizedDiffHash.length > 0 &&
        currentRepresentation.normalizedDiffHash ===
            candidateRepresentation.normalizedDiffHash;
    const filePathHashMatch =
        currentRepresentation.filePathHash.length > 0 &&
        currentRepresentation.filePathHash ===
            candidateRepresentation.filePathHash;
    ...
    const passesStructural =
        structuralSimilarity >= config.structuralSimilarityThreshold;
    const passesMetadata =
        metadataSimilarity >= config.metadataSimilarityThreshold;

    const exactDuplicate =
        patchIdMatch ||
        (normalizedDiffHashMatch && filePathHashMatch && passesHardFilter);
    const metadataDuplicate =
        passesHardFilter && passesStructural && passesMetadata;
...
export function buildDuplicateCommentBody({
    ...
    if (
        ...
        !Array.isArray(detectionResult.matches)
    ) {
        return '';
    }
    ...
    if (detectionResult.reverts && detectionResult.reverts.length > 0) {
        const revertNumbers = detectionResult.reverts
            .map((match) => `#${match.number}`)
            .join(', ');
        lines.push(
            '',
            `Potential reverts detected relative to: ${revertNumbers}.`
        );
    }
...
export async function detectDuplicatePullRequest({
    ...
    for (const candidate of [...openCandidates, ...mergedCandidates]) {
        ...
        if (
            !candidate.base ||
            candidate.base.ref !== currentRepresentation.baseRef
        ) {
            continue;
        }
    ...
    const candidatePool = [...candidateByNumber.values()]
        .sort((left, right) => {
            ...
            const rightIsOpen =
                !right.merged_at && right.state === 'open' ? 0 : 1;
        ...
        async (candidate) => {
            if (typeof candidate.changed_files === 'number') {
                const delta = Math.abs(
                    candidate.changed_files - currentRepresentation.fileCount
                );
            ...
            let candidateRepresentation =
                getCachedRepresentation(candidateCacheKey);
    ...
    const compared = evaluations.filter(
        (value): value is DuplicateMatch => value !== null
    );
    const duplicateMatches = compared
        .filter((entry) => entry.similarity.isDuplicate)
        .sort(
            (left, right) =>
                right.similarity.confidence - left.similarity.confidence
        );
    ...
    const topMatches = duplicateMatches.slice(
        0,
        effectiveConfig.maxReportedMatches
    );
    ...
    if (logger && typeof logger.info === 'function') {
        ...
        if (nearMisses.length > 0) {
            logger.info(
                `  Top ${nearMisses.length} non-duplicate candidate(s) by confidence:`
            );
            for (const entry of nearMisses) {
                ...
                if (!m.patchIdMatch && !m.normalizedDiffHashMatch) {
                    if (m.fileOverlap < t.fileOverlapThreshold)
                        failReasons.push(
                            `file-overlap ${formatPercent(m.fileOverlap)} < ${formatPercent(t.fileOverlapThreshold)}`
                        );
                    if (
                        m.fileOverlap >= t.fileOverlapThreshold &&
                        m.structuralSimilarity < t.structuralSimilarityThreshold
                    )
                        failReasons.push(
                            `structural ${formatPercent(m.structuralSimilarity)} < ${formatPercent(t.structuralSimilarityThreshold)}`
                        );
                    if (
                        m.fileOverlap >= t.fileOverlapThreshold &&
                        m.metadataSimilarity < t.metadataSimilarityThreshold
                    )
                        failReasons.push(
                            `metadata ${formatPercent(m.metadataSimilarity)} < ${formatPercent(t.metadataSimilarityThreshold)}`
                        );
                }
                logger.info(
