...
export function getRepresentationCacheKey({
    ...
    config: Pick<
        DuplicateConfig,
        'maxPatchCharactersPerFile' | 'metadataVectorSize'
    >;
}): string {
    const updatedAt = pullRequest.updated_at || '';
    const headSha =
        pullRequest.head && pullRequest.head.sha ? pullRequest.head.sha : '';
    return [
        owner,
        repo,
        pullRequest.number,
...
export function getCachedRepresentation(
    cacheKey: string
): PullRequestRepresentation | null {
...
export function setCachedRepresentation(
    cacheKey: string,
    representation: PullRequestRepresentation
): void {
    if (representationCache.has(cacheKey)) {
        representationCache.delete(cacheKey);
    }
